...
class OntologyService {
    ...
    async recordDomainEventAndApply(args) {
        const { eventType, logicalId, entityTypeId, entityVersion = 1, data, projectId, actor, idempotencyKey, sourceSystem, sourceRecordId, metadata } = args;
        const now = new Date();
        return this.prisma.$transaction(async (tx) => {
            // 1. Record the Immutable Domain Event
            const event = await (0, domain_events_1.recordDomainEvent)({
                prisma: tx,
                eventType,
                projectId,
                entityTypeId,
     ...
     */
    async replayEntityType(entityTypeId, projectId) {
        return this.prisma.$transaction(async (tx) => {
