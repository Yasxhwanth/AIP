...
async function main() {
    ...
    const executor = new aip_executor_1.AIPExecutor(prisma);
    ...
    try {
        ...
        if (!project)
            throw new Error('Military Operations project not found. Run seed first.');
        ...
        if (!proposalRes.success)
            throw new Error(`Agent proposal failed: ${proposalRes.error}`);
        ...
        if (!cr || cr.status !== 'DRAFT')
            throw new Error(`CR ${crId} not in DRAFT status`);
        ...
        if (!proposalLog)
            throw new Error('Audit log missing for agent proposal.');
        if (!approvalLog)
            throw new Error('Audit log missing for admin approval.');
...
//# sourceMappingURL=verify-platform-e2e.js.map