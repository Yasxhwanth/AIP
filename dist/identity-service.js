 ...
 */
function numericSimilarity(a, b, tolerancePct = 0.05) {
...
class IdentityService {
    ...
    static async runFuzzyMatchJob(entityTypeId, prisma, options = {}) {
        ...
        for (let i = 0; i < instances.length; i++) {
            for (let j = i + 1; j < instances.length; j++) {
                ...
                if (existing)
                    continue;
                const score = scoreEntitySimilarity(a.data, b.data);
    ...
    static async mergeEntities(candidateId, reviewerName, prisma) {
        ...
        if (stateA && stateB) {
            const merged = { ...stateB.data, ...stateA.data }; // A wins on conflicts
