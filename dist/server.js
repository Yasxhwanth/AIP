...
app.post('/api/ontology/entity-types/:id/instances', (0, middleware_1.enforceIdempotency)(prisma), async (req, res) => {
    try {
        ...
        if (!logicalId)
            return res.status(400).json({ error: 'Missing logicalId' });
...
app.get('/api/metrics', async (req, res) => {
    try {
        ...
        if (!projectId)
            return res.status(503).json({ error: 'No project initialised' });
...
app.post('/api/metrics', async (req, res) => {
    try {
        ...
        if (!name || !objectType || !property)
            return res.status(400).json({ error: 'name, objectType, property required' });
    ...
    try {
        ...
        if (currentValue !== null) {
            switch (metric.thresholdOp) {
                case '>':
                    breaching = currentValue > metric.threshold;
                    break;
                case '<':
                    breaching = currentValue < metric.threshold;
                    break;
...
app.get('/api/workflows', async (req, res) => {
    try {
        const projectId = global.DEFAULT_PROJECT_ID;
        if (!projectId)
            return res.status(503).json({ error: 'No project' });
...
async function runAutomation(automationId, triggerType, inputOverride) {
...
(async () => {
    try {
        ...
        schedules.forEach((auto) => auto.cronExpr && scheduleAutomation(auto));
...
app.post('/api/automate/webhook/:path', async (req, res) => {
    try {
        ...
        if (!automation)
            return res.status(404).json({ error: 'No active automation for this webhook path' });
...
app.get('/api/workshop/:id/widget-data', async (req, res) => {
    try {
        ...
        const etByName = Object.fromEntries(entityTypes.map((et) => [et.name.toLowerCase(), et.id]));
        for (const page of pages) {
            for (const section of (page.sections || [])) {
                for (const widget of (section.widgets || [])) {
                    ...
                    const rows = states.map((s) => ({ id: s.logicalId, ...s.data }));
                    if (widget.type === 'object-table' || widget.type === 'loop-layout') {
                        ...
                        const values = prop ? rows.map((r) => parseFloat(r[prop])).filter((v) => !isNaN(v)) : [];
                        const kpiValue = values.length > 0 ? (values.reduce((a, b) => a + b, 0) / values.length) : null;
                        widgetData[widget.id] = {
                            type: widget.type, hasData: rows.length > 0,
                        ...
                        if (prop && rows.length > 0) {
                            ...
                            for (let i = 0; i < Math.min(BUCKETS, rows.length); i++) {
                                ...
                                const vals = slice.map((r) => parseFloat(r[prop])).filter((v) => !isNaN(v));
...
app.post('/api/workshop/query', async (req, res) => {
    try {
        ...
        if (!et)
            return res.json({ rows: [], total: 0, columns: [], hasData: false });
        const states = await prisma.currentEntityState.findMany({
            where: { entityTypeId: et.id },
            orderBy: { updatedAt: 'desc' },
            take: Math.min(limit, 500),
        });
        let rows = states.map((s) => ({ id: s.logicalId, ...s.data }));
...
app.post('/api/analytics/query', async (req, res) => {
    try {
        ...
        if (!et)
            return res.json({ rows: [], columns: [], total: 0 });
        const states = await prisma.currentEntityState.findMany({
            where: { entityTypeId: et.id }, orderBy: { updatedAt: 'desc' }, take: 5000
        });
        let rows = states.map((s) => ({ _id: s.logicalId, ...s.data }));
            ...
            const result = Object.entries(groups).map(([key, vals]) => {
                ...
                switch (aggregation.toUpperCase()) {
                    ...
                    case 'MIN':
                        ...
                        break;
                    case 'MAX':
                        agg = sorted[sorted.length - 1] ?? 0;
                        break;
                    case 'P95':
                        agg = sorted[Math.floor(sorted.length * 0.95)] ?? sorted[sorted.length - 1] ?? 0;
                        break;
...
app.post('/api/analytics/timeseries', async (req, res) => {
    try {
        ...
        if (!entityType || !property)
            return res.status(400).json({ error: 'entityType + property required' });
        const et = await prisma.entityType.findFirst({ where: { name: { equals: entityType, mode: 'insensitive' } } });
        if (!et)
            return res.json({ series: [], property, entityType });
        const states = await prisma.currentEntityState.findMany({
            where: { entityTypeId: et.id }, orderBy: { updatedAt: 'desc' }, take: 2000
        });
        let rows = states.map((s) => ({ ...(s.data), _updatedAt: s.updatedAt }));
        for (const f of filters) {
            rows = rows.filter((r) => {
                const v = r[f.property];
        ...
        rows.sort((a, b) => new Date(a._updatedAt).getTime() - new Date(b._updatedAt).getTime());
        const bucketSize = Math.max(1, Math.ceil(rows.length / buckets));
        const series = [];
        for (let i = 0; i < Math.min(buckets, rows.length); i++) {
            const slice = rows.slice(i * bucketSize, (i + 1) * bucketSize);
            const vals = slice.map((r) => parseFloat(r[property])).filter((v) => !isNaN(v));
...
app.get('/api/analytics/entity-summary', async (req, res) => {
    try {
        const entityTypes = await prisma.entityType.findMany({ select: { id: true, name: true } });
        const summary = await Promise.all(entityTypes.map(async (et) => {
            const count = await prisma.currentEntityState.count({ where: { entityTypeId: et.id } });
...
app.post('/api/actions', async (req, res) => {
    try {
        ...
        if (!name || !objectType)
            return res.status(400).json({ error: 'name, objectType required' });
...
app.post('/api/actions/:id/execute', async (req, res) => {
    try {
        ...
        if (!logicalId)
            return res.status(400).json({ error: 'logicalId of the target entity is required' });
        // Validate required params
        const paramDefs = action.params;
        const missingParams = paramDefs
            .filter((p) => p.required && (parameters[p.name] === undefined || parameters[p.name] === null || parameters[p.name] === ''))
            .map((p) => p.name);
        ...
        const needsApproval = action.approvalRules.length > 0 && action.riskTier !== 'low';
            ...
            try {
                ...
                if (currentState) {
                    ...
                    if (category === 'edit') {
                        ...
                        const writesToFields = action.writesTo;
                        for (const param of paramDefs) {
                            if (parameters[param.name] !== undefined) {
                                ...
                                if (fieldName)
                                    updatedData[fieldName] = parameters[param.name];
                            }
                        }
                    }
                ...
                logger_1.default.warn({ writeErr, logicalId }, 'Could not apply write-back to CurrentEntityState');
            }
        }
        ...
        return res.json({
            ...
            message: needsApproval
                ? `Action submitted for approval. ${action.approvalRules.length} approval rule(s) apply.`
...
app.post('/api/entities/publish', async (req, res) => {
    try {
        ...
        if (!objectType || !logicalId)
            return res.status(400).json({ error: 'objectType and logicalId required' });
...
function broadcastEntityChange(objectType, logicalId, data, changeType = 'updated') {
    broadcastToTopics([`entities:${objectType}`, 'entities:*', 'events:*'], { type: 'entity.change', changeType, objectType, logicalId, data, ts: Date.now() });
    // Also fire the automate event dispatcher
    const dispatch = global.dispatchAutomateEvent;
    if (dispatch) {
        dispatch(global.DEFAULT_PROJECT_ID, `entity.${changeType}`, { objectType, logicalId, data }).catch(() => { });
    }
}
/** Called after a metric threshold breach */
function broadcastMetricAlert(metricId, metricName, value, threshold) {
    broadcastToTopics([`metrics:${metricId}`, 'metrics:*', 'events:*'], { type: 'metric.threshold_breached', metricId, metricName, value, threshold, ts: Date.now() });
}
/** Called after an action execution */
function broadcastActionEvent(actionName, logicalId, executionId, status) {
    broadcastToTopics(['actions:*', 'events:*'], { type: 'action.executed', actionName, logicalId, executionId, status, ts: Date.now() });
}
...
function initWebSocketServer(httpServer) {
    ...
    wss.on('connection', (ws, req) => {
        ...
        const client = { ws, id: clientId, subscriptions: new Set(['events:*']), connectedAt: new Date() };
        wsClients.set(clientId, client);
        logger_1.default.info(`[WS] Client connected: ${clientId} (total: ${wsClients.size})`);
        // Send welcome
        ...
        ws.on('message', (raw) => {
            try {
                ...
                if (msg.subscribe) {
                    ...
                    const topics = Array.isArray(msg.subscribe) ? msg.subscribe : [msg.subscribe];
                ...
                if (msg.unsubscribe) {
                    const topics = Array.isArray(msg.unsubscribe) ? msg.unsubscribe : [msg.unsubscribe];
        ...
        ws.on('close', () => {
            ...
            logger_1.default.info(`[WS] Client disconnected: ${clientId} (remaining: ${wsClients.size})`);
        });
        ws.on('error', (err) => {
            logger_1.default.warn({ err }, `[WS] Client error: ${clientId}`);
            wsClients.delete(clientId);
        });
    });
...
app.post('/api/evals/:id/run', async (req, res) => {
    try {
        ...
        (async () => {
            const cases = evaluation.testCases;
...
app.get('/api/lineage/:entityType', async (req, res) => {
    try {
        ...
        const upstreamPipelines = pipelines.filter(p => {
            ...
            return pnodes?.some((n) => n.type === 'ontologyWrite' && n.data?.entityType === name);
        });
        // Find pipelines that read from this entity type
        const downstreamPipelines = pipelines.filter(p => {
            const pnodes = p.nodes;
            return pnodes?.some((n) => n.type === 'restFetch' && String(n.data?.url).includes(name));
        });
...
app.get('/api/provenance/verify/:entityId', async (req, res) => {
    try {
        const result = await provenance_service_1.ProvenanceService.verifyIntegritySeal(req.params.entityId, prisma);
        return res.json(result);
    }
    catch (err) {
        if (String(err).includes('No integrity seal'))
            return res.status(404).json({ error: 'No seal found' });
        return res.status(500).json({ error: String(err) });
    }
});
