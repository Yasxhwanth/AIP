...
const strategies = {
    ...
    THRESHOLD: async (input, params) => {
        const { field, operator, threshold } = params;
    ...
    ANOMALY_ZSCORE: async (input, params, context) => {
        const { field, lookbackMinutes, zThreshold } = params;
    ...
    CUSTOM: async (input, params) => {
        ...
        try {
            const value = (0, computed_metrics_1.evaluateExpression)(expression, input);
            return {
 ...
 */
async function runInferenceByModel(modelDefinitionId, logicalId, prisma) {
