...
import { OntologyRebuilder } from './ontology-rebuilder';
...
import { createGovernanceRouter } from './routers/governance-router';
import { createAgentRouter } from './routers/agent-router';
import { createHealthRouter } from './routers/health-router';
import { createMavenRouter } from './routers/maven-router';
import { createOntologyRouter } from './routers/ontology-router';
import { executeWorkflow } from './workflow-engine';
import { executePipeline } from './pipeline-engine';
import { GovernanceService } from './governance-service';
import { OntologyService } from './ontology-service';
import { BulkIngestionService } from './services/bulk-ingestion-service';
// import { pool } from './db';
import { OutboxService } from './outbox-service';
import { startDroneTelemetry } from './drone-telemetry-service';
...
// Initialize engines
const abac = new AbacEngine(prisma);
...
const rebuilder = new OntologyRebuilder(prisma);
const qualitySvc = new QualityService(prisma);
...
app.use(securityGuard(securityCtx));
...
app.use('/api/v1/aip', createAipRouter(prisma));
app.use('/api/governance', createGovernanceRouter(prisma));
app.use('/api/v1/maven', createMavenRouter(prisma));
app.use('/api/v1/agents', createAgentRouter(prisma));
app.use('/api/v1/ontology', createOntologyRouter(prisma));
...
app.get('/api/v1/telemetry/jobs', async (req, res) => {
  try {
    ...
    const summary = await (prisma as any).jobQueue.groupBy({
      by: ['status'],
      where: { projectId },
      _count: true
    });

    const activeWorkers = await (prisma as any).jobWorker.count({
      where: {
        status: 'ACTIVE',
    ...
    const recentJobs = await (prisma as any).jobQueue.findMany({
      where: { projectId },
      orderBy: { createdAt: 'desc' },
      take: 50,
    ...
    return res.json({
      summary: summary.reduce((acc: any, curr: any) => ({ ...acc, [curr.status]: curr._count }), {}),
      activeWorkers,
      recentJobs
    });
...
app.get('/api/v1/telemetry/agents', async (req, res) => {
  try {
    ...
    const toolStats: Record<string, { success: number; failed: number; total: number; durSum: number; p90Samples: number[] }> = {};
    ...
    logs.forEach(log => {
      ...
      if (status === 'SUCCESS') toolStats[toolName].success++;
      else toolStats[toolName].failed++;
    ...
    return res.json({
      summary: {
        ...
        successRate: logs.length > 0 ? (logs.filter(l => (l.metadata as any)?.status === 'SUCCESS').length / logs.length) * 100 : 0,
        avgLatency: logs.length > 0 ? logs.reduce((acc, l) => acc + ((l.metadata as any)?.durationMs || 0), 0) / logs.length : 0
      },
      metricsByTool,
...
app.post('/api/v1/change-requests', async (req, res) => {
  try {
    ...
    if (!resourceType || !proposedChanges) return res.status(400).json({ error: 'resourceType and proposedChanges are required' });
...
app.get('/api/v1/telemetry/api-latency', async (req, res) => {
  try {
    ...
    const buckets: { label: string; avgLatency: number; errorRate: number; count: number; errorCount: number; sumLatency: number }[] = [];
    for (let i = NUM_BUCKETS - 1; i >= 0; i--) {
      ...
      for (const log of logs) {
        ...
        if (ts >= bucketStart && ts < bucketEnd) {
          buckets[idx].count++;
          if (log.status === 'FAILED' || log.status === 'DENIED') buckets[idx].errorCount++;
...
// ── Audit Log Telemetry ────────────────────────────────────────────
app.get('/api/v1/telemetry/audit', async (req, res) => {
  try {
    let projectId = req.auth?.projectId || (req.query.projectId as string) || req.header('X-Project-Id');
    if (!projectId && process.env.NODE_ENV !== 'production') projectId = (global as any).DEFAULT_PROJECT_ID;
...
// ── POST /api/ontology/entity-types/:id/instances/bulk — create many data rows ──
app.post('/api/ontology/entity-types/:id/instances/bulk', enforceIdempotency(prisma), async (req, res) => {
  try {
    const entityTypeId = req.params.id as string;
    ...
    // 1. Solidify JobRun Lifecycle (Tracking in DB via JobQueue)
    const job = await prisma.jobQueue.create({
      data: {
        ...
        payload: {
          entityTypeId,
          projectId,
          actor: actor || req.auth?.apiKeyName || 'system',
          items
        }
      }
    });

    // 2. Offload work to RabbitMQ Worker
    if (amqpChannel) {
      const payload = JSON.stringify({ type: 'bulk', jobId: job.id, ...job.payload as any });
      amqpChannel.sendToQueue(QUEUE_NAME, Buffer.from(payload), { persistent: true });

      return res.status(202).json({
        message: 'Bulk ingestion payload accepted for background processing',
        jobId: job.id,
        status: 'queued'
      });
    } else {
      ...
      const result = await bulkSvc.execute(job.payload as any);
...
app.get('/api/governance/change-requests', async (req, res) => {
  try {
    const projectId = req.auth?.projectId || (req.query.projectId as string);
...
app.post('/api/governance/change-requests/:id/approve', async (req, res) => {
  try {
    ...
    logger.error({ crId: req.params.id, error: String(err) }, 'Failed to approve change request');
    return res.status(500).json({ error: String(err) });
  }
});
...
app.get('/api/ontology/change-requests', async (req, res) => {
  try {
    const projectId = req.auth?.projectId || (req.query.projectId as string);
    const crs = await governanceSvc.listChangeRequests(projectId || '', req.query.status as string);
    return res.json(crs);
  } catch (err) {
    return res.status(500).json({ error: String(err) });
  }
...
app.post('/api/ontology/reason', async (req, res) => {
  try {
    ...
    const job = await orchestrator.enqueue(
      'SEMANTIC_REASONING',
      { projectId },
      { projectId, idempotencyKey: `semantic-reasoning:${projectId}:${Date.now()}` }
    );
    return res.status(202).json({
...
app.get('/api/v1/alerts', async (req, res) => {
  try {
    ...
    if (!projectId) {
      return res.status(401).json({ error: 'Project context required' });
    }
...
app.post('/entity-types/:id/instances', async (req, res) => {
  try {
    ...
    const previousState = (event.payload as any)?.previousState ?? null; // OntologyService provides this in the payload if implemented, or we can assume null if it's a fresh creation focus
...
app.post('/entity-types/:id/instances/bulk', async (req, res) => {
  try {
    ...
    if (!evalResult.allowed) {
      logger.warn({ actor, entityTypeId: req.params.id, reason: evalResult.reason }, "ABAC Deny on Bulk Ingestion");
    ...
    const bulkProjectId = req.auth?.projectId ?? req.header('X-Project-Id') ?? (global as any).DEFAULT_PROJECT_ID;
    const bulkJob = await orchestrator.enqueue(
      'BULK_INGESTION',
      { entityTypeId: entityType.id, projectId: bulkProjectId, actor: req.auth?.apiKeyName ?? 'api', items },
      { projectId: bulkProjectId, idempotencyKey: `bulk-ingest:${entityType.id}:${Date.now()}`, priority: 8 }
    );
    return res.status(202).json({
...
app.get('/entity-types/:id/instances', async (req, res) => {
  try {
    ...
    if (!evalResult.allowed) {
      logger.warn({ actor, entityTypeId: req.params.id, reason: evalResult.reason }, "ABAC Deny on Entity Instances list");
      return res.status(403).json({ error: 'Access Denied', reason: evalResult.reason });
    }
...
app.post('/integration-jobs/:id/execute', enforceIdempotency(prisma), async (req, res) => {
  try {
    ...
    const jobDef = await prisma.integrationJob.findUnique({ where: { id: req.params.id as string } });
    if (!jobDef) return res.status(404).json({ error: 'Integration job not found' });
...
app.post('/api/v1/identity/candidates/:id/merge', async (req, res) => {
  try {
    const candidate = await (prisma as any).matchCandidate.findUnique({ where: { id: req.params.id } });
    if (!candidate) return res.status(404).json({ error: 'MatchCandidate not found' });

    const reviewerName = req.auth?.apiKeyName ?? 'system';
    await IdentityService.mergeEntities({
  ...
  try {
    ...
    for (const id of candidateIds) {
      try {
        ...
        const currentCandidate = candidate as any;
        await IdentityService.mergeEntities({
          candidateId: id,
          reviewerName,
    ...
    if (!needsApproval) {
      try {
        ...
        if (currentState) {
          ...
          // Update the CurrentEntityState projection using the record-and-apply pattern
          await ontologySvc.recordDomainEventAndApply({
            eventType: 'EntityUpdated',
            logicalId,
            entityTypeId: currentState.entityTypeId,
            data: updatedData,
            projectId: currentState.projectId || (global as any).DEFAULT_PROJECT_ID,
  ...
  try {
    ...
    if (data) {
      ...
      if (entityType && projectId) {
        const ontologySvc = new (require('./ontology-service').OntologyService)(prisma);
        await ontologySvc.recordDomainEventAndApply({
          eventType: changeType === 'created' ? 'EntityCreated' : 'EntityStateChanged',
          logicalId,
          entityTypeId: entityType.id,
          data,
          projectId,
          actor: 'external-publish'
        });
      }
    }
...
app.get('/api/v1/telemetry/jobs', async (req, res) => {
  try {
    ...
    const recentJobs = await prisma.jobQueue.findMany({
      ...
      take: 50,
      include: {
        integrationJob: { select: { name: true } }
      }
    });
    ...
    return res.json({ summary, activeWorkers, recentJobs });
  } catch (error) {
    return res.status(500).json({ error: String(error) });
  }
});
...
app.get('/api/v1/telemetry/api-latency', async (req, res) => {
  return res.json({
    buckets: Array.from({ length: 20 }).map((_, i) => ({
      label: `-${20 - i}m`,
      avgLatency: 50 + Math.random() * 20,
      errorRate: Math.random() < 0.1 ? Math.random() * 2 : 0
    }))
  });
});
