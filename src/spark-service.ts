...
export class SparkService {
     ...
     */
    async executeJob(jobId: string, trigger: string, inputData: any = null, broadcastFn?: (eventUrl: string, payload: any) => void) {
        const job = await this.prisma.sparkJob.findUnique({ where: { id: jobId } });
        if (!job) throw new Error("Spark Job not found");

        const stages = (job.stages as any[]) || [];
    ...
    private async processDag(job: any, run: any, stages: any[], broadcastFn?: Function) {
        ...
        for (const stg of stages) {
            ...
            if (broadcastFn) broadcastFn(`spark:job:${job.id}`, { type: "stage.started", runId: run.id, stageId: dbStage.stageId });
            ...
            let duration = 500;
            switch (stg.type) {
                case "source": duration = 1200; break;
                case "filter": duration = 800; break;
                case "join": duration = 2500; break;
                case "aggregate": duration = 1800; break;
            }
            ...
            if (Math.random() < 0.02) {
                ...
                jobFailed = true;
                if (broadcastFn) broadcastFn(`spark:job:${job.id}`, { type: "stage.failed", runId: run.id, stageId: dbStage.stageId });
                break;
            } else {
                // Success
                const recordsIn = Math.floor(Math.random() * 100000) + 5000;
                let recordsOut = recordsIn;
                if (stg.type === "filter") recordsOut = Math.floor(recordsIn * (Math.random() * 0.5 + 0.1));
                if (stg.type === "aggregate") recordsOut = Math.floor(recordsIn * 0.05);
                ...
                if (broadcastFn) broadcastFn(`spark:job:${job.id}`, { type: "stage.success", runId: run.id, stageId: dbStage.stageId });
            }
        }
        ...
        if (broadcastFn) broadcastFn(`spark:job:${job.id}`, { type: jobFailed ? "job.failed" : "job.success", runId: run.id });
    }
}
