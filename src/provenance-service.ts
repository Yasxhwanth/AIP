...
export class ProvenanceService {
     ...
     */
    static async recordCryptoProvenance(
        entityId: string,
        entityType: string,
        ...
        for (const [field, value] of Object.entries(fields)) {
            ...
            const valueHash = crypto.createHash('sha256').update(valueStr).digest('hex');
    ...
    static async createIntegritySeal(entityId: string, entityType: string, sealedBy: string, projectId: string, prisma: PrismaClient) {
        ...
        const hmac = crypto.createHmac('sha256', this.getSecretKey()).update(sealData).digest('hex');
     ...
     */
    static async verifyIntegritySeal(entityId: string, prisma: PrismaClient) {
        const seal = await prisma.cryptoProvenanceSeal.findFirst({
            where: { entityId },
            orderBy: { sealedAt: 'desc' }
        });

        if (!seal) throw new Error(`No integrity seal found for entity ${entityId}`);

        // Recompute HMAC based on the frozen snapshot
        const hashes = seal.fieldHashes as Record<string, string>;
        const fieldKeys = Object.keys(hashes).sort();
        const sealData = fieldKeys.map(k => `${k}:${hashes[k]}`).join('|');

        const expectedHmac = crypto.createHmac('sha256', this.getSecretKey()).update(sealData).digest('hex');
        ...
        await prisma.cryptoProvenanceSeal.update({
            ...
            data: {
                ...
                tamperedFields: tamperedFields ? tamperedFields : undefined as any
            }
        });
