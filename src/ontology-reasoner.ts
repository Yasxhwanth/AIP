...
interface AntecedentHop {
    relDefId: string;    // RelationshipDefinition id
    direction: 'outgoing' | 'incoming';
}

interface Consequent {
    relDefId: string;    // RelationshipDefinition id for derived relationship
    confidence: number;  // Base confidence for the derived relationship (0.0-1.0)
    decayRate: number;   // Confidence decay rate per day (0.0 = no decay)
}

interface OntologyRuleData {
    id: string;
    name: string;
    antecedent: AntecedentHop[];
 ...
 */
async function traverseHop(
    startLogicalIds: string[],
    hop: AntecedentHop,
    prisma: PrismaClient,
    ...
    if (startLogicalIds.length === 0) return results;
 ...
 */
async function traverseRule(
    sourceLogicalId: string,
    rule: OntologyRuleData,
    prisma: PrismaClient,
): Promise<Array<{ targetLogicalId: string; pathConfidence: number }>> {
    // Track { logicalId -> accumulated path confidence }
    let currentLayer = new Map<string, number>([[sourceLogicalId, 1.0]]);
...
async function upsertDerivedRelationship(
    rule: OntologyRuleData,
    sourceLogicalId: string,
    targetLogicalId: string,
    ...
    const derivedConfidence = Math.min(
        (rule.consequent.confidence ?? 1.0) * pathConfidence,
        1.0,
    );
 ...
 */
export async function runReasonerForEntity(
    logicalId: string,
    projectId: string,
    prisma: PrismaClient,
    ...
    for (const rule of rules) {
        const antecedent = rule.antecedent as unknown as AntecedentHop[];
        const consequent = rule.consequent as unknown as Consequent;
        const ruleData: OntologyRuleData = {
            id: rule.id,
            name: rule.name,
            antecedent,
            consequent,
 ...
 */
export async function runFullReasoner(
    projectId: string,
    prisma: PrismaClient,
): Promise<{
    rulesRun: number;
