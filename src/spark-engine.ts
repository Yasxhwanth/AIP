 ...
 */
export async function executeSparkJob(
    jobId: string,
    runId: string,
    prisma: PrismaClient,
    broadcastFn?: (eventUrl: string, payload: any) => void
) {
    ...
    if (!run) throw new Error("Spark Job Run not found");
    ...
    for (const stg of stages) {
        ...
        if (broadcastFn) broadcastFn(`spark:job:${jobId}`, { type: "stage.started", runId: run.id, stageId: dbStage.stageId });
        ...
        if (Math.random() < 0.02) {
            ...
            jobFailed = true;
            if (broadcastFn) broadcastFn(`spark:job:${jobId}`, { type: "stage.failed", runId: run.id, stageId: dbStage.stageId });
            break;
        } else {
            ...
            if (broadcastFn) broadcastFn(`spark:job:${jobId}`, { type: "stage.success", runId: run.id, stageId: dbStage.stageId });
        }
    }
    ...
    if (broadcastFn) broadcastFn(`spark:job:${jobId}`, { type: jobFailed ? "job.failed" : "job.success", runId: run.id });
